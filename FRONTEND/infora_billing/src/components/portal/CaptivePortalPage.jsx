...
function WelcomeBanner({ config, query }) {
  ...
  return (
    <PortalFadeIn>
      <div
        className={`rounded-2xl border p-6 sm:p-8 ${isLight ? 'border-slate-200 bg-white shadow-sm' : 'border-white/10 bg-white/[0.04]'}`}
      >
        ...
        <div className={`mt-5 flex flex-wrap items-center gap-4 text-xs ${cx.textMuted}`}>
        ...
        {pppoeEnabled && (
          <p className={`mt-4 text-sm ${cx.textMuted}`}>
            Home subscriber?{' '}
            <Link to={`/portal/pppoe${query}`} className="font-medium hover:underline" style={{ color: accent }}>
              Check your account →
            </Link>
          </p>
        )}
          ...
          <div className="space-y-10">
            ...
            <PortalFadeIn delay={0.08}>
              <PortalGlassCard>
                ...
                <p className="mt-1 text-sm opacity-70">Three steps to get browsing.</p>
                <div className="mt-5">
