...
export default function DualWanPanel({ deviceId, device, onApplied }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {isOn && (
        <>
          {/* Promoting a LAN port to a WAN is supported — it is what you do when a
              second uplink arrives. So this states the consequence and lets the
              operator proceed, rather than refusing on a port role that goes
              stale the moment someone rewires. The router's own verification is
              the real gate: an uplink with nothing upstream fails it, and the
              rollback guard puts the router back. */}
          {lanPortWarnings.length > 0 && (
            <div className="mt-4 flex gap-2 rounded-xl bg-amber-50 border border-amber-200 p-3 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-950/30 dark:text-amber-300">
              <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
              <span>
                {lanPortWarnings.join(', ')} {lanPortWarnings.length === 1 ? 'is currently a LAN port' : 'are currently LAN ports'}.
                {' '}Applying this removes {lanPortWarnings.length === 1 ? 'it' : 'them'} from the bridge to act as an uplink,
                and anything connected to {lanPortWarnings.length === 1 ? 'that port' : 'those ports'} loses service until
                {' '}{lanPortWarnings.length === 1 ? 'it is' : 'they are'} put back.
                {' '}<strong>Disable on router</strong> restores {lanPortWarnings.length === 1 ? 'it' : 'them'} to the bridge.
                {' '}Make sure a real uplink is plugged in first — a WAN with nothing upstream fails verification and is rolled back.
              </span>
            </div>
          )}
