...
export default function LumenLogo({
  ...
  const subtitleText = subtitle ?? 'WiFi Billing';
  ...
  const subClass = theme === 'dark' ? 'text-slate-400' : 'text-cyan-600/80';
  ...
  return (
    <div className={`inline-flex items-center gap-3 ${className}`}>
      ...
      <div className="min-w-0 text-left">
        ...
        {subtitleText && (
          <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${subClass}`}>
            {subtitleText}
          </p>
        )}
      </div>
